...
class MainRecommender:
    ...
    def get_similar_items_recommendation(self, user, N=5):
        ...
        #     res_similar = []
        #     for item in top_items_bought:
        #         recs = model.similar_items(self.itemid_to_id[item], N=2)
        #         top_rec = recs[1][0]
        #         res_similar.append(self.id_to_itemid[top_rec])
    ...
    def get_similar_users_recommendation(self, user, N=5):
        ...
        for user_in_model in similar_users:
            user_in_matrix = self.id_to_userid[user_in_model[0]]
            popular = self.user_item_matrix.loc[user_in_matrix].sort_values(ascending=False).reset_index().head(2)
            popular = popular.loc[popular['item_id'] != 999999]
